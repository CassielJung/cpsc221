...
/* Helper function */
string AnagramDict::f(string &word) const {
    // convert to vector
...
{
    string temp = word;
    auto it = dict.find(f(temp));
    if (it != dict.end()) {
        it->second.push_back(word);
    } else {
        dict[f(temp)].push_back(word);
    }
...
vector<vector<string>> AnagramDict::get_all_anagrams() const
{
    ...
    anagrams.resize(dict.size());

    int index = 0;
    for (auto it : dict) {
        anagrams[index] = it.second;
    }
    return anagrams;
}
