...
T Queue<T>::Dequeue()
{
  // if "outbox" stack is empty, copy every element in "inbox" to "outbox"
  if (stack_2.IsEmpty()) {
...
T Queue<T>::Peek()
{
  // if "outbox" stack is empty, copy everything from "inbox" to "outbox"
  if (stack_2.IsEmpty()) {
    while (!stack_1.IsEmpty()) {
      stack_2.Push(stack_1.Pop());
    }
