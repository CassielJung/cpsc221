...
void sketchify(std::string inputFile, std::string outputFile) {
    ...
    cout << "Reached line 46" << endl; // debug

    original->readFromFile(inputFile);
    unsigned width = original->width();
    unsigned height = original->height();

    cout << "Reached line 50" << endl; // debug
    ...
    cout << "setting output done" << endl; //debug

    // Save the output file
    output->writeToFile(outputFile);
    cout << "save output file done" << endl; //debug
    // Clean up memory
    delete myPixel;
    cout << "delete myPixel done" << endl; //debug
    delete output;
    delete original;
}
